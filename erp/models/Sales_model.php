...
class Sales_model extends CI_Model
{
	...
	public function getProductNumber($term, $warehouse_id, $standard, $combo, $digital, $service, $category, $limit = 20)
    {
		if(preg_match('/\s/', $term))
		{
			$name = explode(" ", $term);
			$first = $name[0];
			$this->db->select('products.id, code, name, type, cost,warehouses_products.quantity, price, tax_rate, tax_method, cf1, product_details,subcategory_id, details,COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) 
					FROM erp_serial as sp
				 WHERE sp.product_id='.$this->db->dbprefix('products').'.id
				), "") as sep')
            ->group_by('products.id');
    ...
    public function getMaxqtyByProID($pid){
        $this->db->select('MAX(qty_unit) as mva');
	...
	public function getVarain($qtyunit){
        $this->db->select('*');
        $q = $this->db->get_where('product_variants', array('qty_unit' => $qtyunit));
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
	...
	public function addSale($data = array(), $items = array(), $payment = array(), $loans = array(),$deliver_id_muti=NULL)
	{
		...
		if ($this->db->insert('sales', $data)) {
			...
			if ($data['sale_status'] == 'completed') {
				//$this->erp->print_arrays($deliver_id_muti);
    ...
    public function getOrderDeliveryByID($id=null,$wh=null){
		...
		return false;
  //   	$this->db->select("deliveries.customer_id,deliveries.id as id, deliveries.date, deliveries.do_reference_no, deliveries.sale_reference_no,deliveries.sale_reference_no AS reference_no , companies.name as customer_name, deliveries.address, COALESCE(SUM(erp_delivery_items.quantity_received),0) as qty, deliveries.sale_status,deliveries.sale_id,deliveries.biller_id,deliveries.delivery_by,sale_order.*")
  //           ->from('deliveries')
		// 	->where('type','sale_order')
  //           ->join('delivery_items', 'delivery_items.delivery_id = deliveries.id', 'left')
		// 	->join('sale_order','sale_order.id = deliveries.id','left')
  ...
  //           ->group_by('deliveries.id')
    ...
    {
        $this->db->select('*,sales.reference_no as reslae, CONCAT(erp_users.first_name," ",erp_users.last_name) AS name')
    ...
    public function addPayment($data = array())
    {
        ...
        if ($this->db->insert('payments', $data)) {
			...
			$this->db->update('sales', array('recieve_usd' => $data['pos_paid']), array('id' => $data['sale_id']));
	...
	public function getSaleOrder($sale_order_id){
		$this->db->select("sale_order.*, companies.name, companies.company,
			...
			WHEN 'pending' THEN
				'Order'
			END AS status, tax_rates.rate AS tax,tax_rates.name");
	...
	public function Addloans($loans=null,$sale_id=null,$biller_id=null,$frequency=null,$term=null,$depre_type=null,$princ_type=null)
	{
		if($loans){
			foreach($loans as $loan){
				$this->db->insert('loans', $loan);
			...
			$this->db->update('sales', array('biller_id'=>$biller_id,'term' => $term,'frequency'=>$frequency,'depreciation_type'=>$depre_type,'principle_type'=>$princ_type), array('id' => $sale_id));
