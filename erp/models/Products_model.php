...
class Products_model extends CI_Model
{
    ...
    public function addProduct($data, $items, $product_attributes, $photos, $related_products=NULL)
    {
    ...
    public function updateProduct($id, $data, $items, $product_attributes, $photos, $update_variants, $related_products=NULL)
    {
	...
	public function insert_enter_using_stock($data, $ref_prefix)
	{
		if($this->db->insert('enter_using_stock', $data)){
            ...
            if ($this->site->getReference($ref_prefix, $data['shop']) == $data['reference_no']) {
    ...
    public function addStockCount($data)
    {
        if ($this->db->insert('stock_counts', $data)) {
			...
			if($this->Admin || $this->Owner){
				$data['biller_id'] = $this->site->get_setting()->default_biller;
			}
    ...
    {
		//$this->erp->print_arrays($data, $dataPurchase);
        if ($this->db->update('adjustments', $data, array('id' => $id))) {
			$quantity_balance = 0;
			$pro_db_id  = array();
			$pro_in_id  = array();
			...
			//$this->erp->print_arrays($pro_unique);
			
			$this->db->delete('adjustment_items', array('adjust_id'=>$id));
			
			foreach($itemid as $idd){
				$this->db->delete("inventory_valuation_details",array('field_id'=>$idd['itemidd']));
			}
			foreach($adjust_item as $adj_items){
				$this->db->delete('purchase_items', array('transaction_id'=>$adj_items->id));
			}
    ...
    {
        $this->db->select("warehouses.id as id, sale_order.reference_no, warehouses.name, (erp_sale_order_items.quantity - erp_sale_order_items.quantity_received) as qty, sale_order.sale_status")
            ...
            ->where("(erp_sale_order.sale_status ='order' OR erp_sale_order.delivery_status <> 'completed')", NULL, FALSE);
