...
ORDER BY
    ...
    public function getDailySales($year, $month)
    {
		...
		SUM( COALESCE( erp_sales.product_discount, 0 ) ) AS discount, 
    ...
    public function getStaffDailySales($user_id, $year, $month)
    {
        $myQuery = "SELECT DATE_FORMAT( date,  '%e' ) AS date, SUM( COALESCE( product_tax, 0 ) ) AS tax1, 
			...
			SUM( COALESCE( product_discount, 0 ) ) AS discount,
...
*/
	...
	public function getSupplierByID($id){
		...
		$this->db
				 ->select($this->db->dbprefix('companies') . ".id as idd, companies.company, name, companies.phone, companies.email, count(" . $this->db->dbprefix('purchases') . ".id) as total, COALESCE(sum(grand_total), 0) as total_amount, COALESCE(sum(paid), 0) as paid, ( COALESCE(sum(grand_total), 0) - COALESCE(sum(paid), 0)) as balance,CONCAT(erp_users.first_name,' ',erp_users.last_name) as create_by", FALSE)
                ->from("companies")
    ...
    function getQuantityByID($id,$wareid){
		if($wareid){
			 $this->db
					...
					->where(array("warehouses_products.warehouse_id"=>$wareid, 'products.id' => $id))
                   ->group_by('products.id');
		}else{
			$this->db
             ->select('erp_products.id, image, code, name, warehouses_products.quantity, alert_quantity')
	...
	*/
	...
	public function getProPur($wid,$cid,$product2){
		$this->db->select("product_id,products.code,products.name,units.name as name_unit,products.category_id,warehouse_id")
		->join("products","products.id=purchase_items.product_id","LEFT")
	...
	public function getQtyByWare($pid,$wid,$product2,$category2,$start,$end){
		$this->db->select("SUM(COALESCE(erp_purchase_items.quantity_balance,0)) as wqty");
		$this->db->join("purchase_items","purchase_items.product_id=products.id","LEFT");
		if($product2){
			$this->db->where("products.id",$product2);
		}
		if($category2){
	...
	public function getAllProductsDetail1($pid,$cid){
		$this->db->select("products.*,units.name as uname");
		$this->db->join("units","units.id=products.unit","LEFT");
		if($pid){
			$this->db->where("products.id",$pid);
	...
	public function getStockINOUTMS($product,$category,$warehouse,$wid,$in_out,$year){
		$this->db->select("purchase_items.product_id,products.code,products.name,purchase_items.date,DATE_FORMAT(erp_purchase_items.date, '%Y-%m') as dater,DATE_FORMAT(erp_purchase_items.date, '%m') as datm,units.name as name_unit")
		->join("products","products.id=purchase_items.product_id","LEFT")
		->join("units","units.id=products.unit","LEFT")
		->where("DATE_FORMAT(erp_purchase_items.date, '%Y')=",$year);
	...
	public function getSalePaidInvoice($start_date = NULL, $end_date = NULL, $per_page, $ob_set)
	{
