...
class Pos_model extends CI_Model
{
    ...
    {
        $this->db->select('products.id,products.cost, products.code, products.name, products.type,categories.type AS cate_type,warehouses_products.product_id, warehouses_products.quantity, price, tax_rate, tax_method,products.image,subcategory_id,cf1, COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) 
    ...
    {
        $this->db->select('sale_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, product_variants.name as variant, products.name_kh,products.id AS pro_id,products.name AS pro_name')
    ...
    {
        $this->db->select('sales.*, users.username,erp_tax_rates.name AS tax,erp_payments.paid_by,erp_users.phone,erp_payments.cheque_no,erp_payments.cc_no,erp_payments.cc_type,erp_warehouses.name AS ware, erp_payments.pos_balance, erp_payments.pos_paid_other_rate');
    ...
    */
	...
	public function getsuspendedNULL($id)
    {
		...
		$this->db->select("erp_suspended.*")
				 ->from('erp_suspended')
				 ->where('id!=', $id)
				 ->where_in('warehouse_id',$warehouses);
				$q = $this->db->get();
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
