...
class Site extends CI_Model
{
	...
	public function getCompanyByID($id) 
	{
        $this->db->select("companies.id,
							...
							company, company_kh, name,name_kh, email, phone, 
							group_areas.areas_group, 
							customer_group_name, customer_group_id, vat_no,
							price_group_id,erp_companies.address, erp_companies.city, erp_companies.postal_code,erp_companies.street,erp_companies.village,erp_companies.sangkat,erp_companies.district, erp_companies.state, erp_companies.country,
    ...
    public function getProductByID($id) {
        $this->db->select('products.*,products.quantity AS qoh, units.name as unit, products.unit as unit_id,categories.type as cate_type, product_variants.qty_unit');
	...
	public function updateReference($field, $biller=NULL) {
		...
		if($this->session->userdata('biller_id')){
			$default_biller = $this->session->userdata('biller_id');
		}else{
		...
		if($this->get_setting()->separate_ref == 1){
			...
			if($biller){
				...
				if ($this->get_setting()->reference_format == 4 || $this->get_setting()->reference_format == 5) {
					$q = $this->db->get_where('order_ref',array('biller_id'=>$default_biller), 1);
					if ($q->num_rows() > 0) {
						$ref = $q->row();
						$this->db->update('order_ref', array($field => $ref->{$field} + 1), array('biller_id'=>$default_biller));
						return TRUE;
					}
					return FALSE;
	...
	public function getWarehouseQty($product_id, $warehouse_id){
		$this->db->select("erp_warehouses_products.*");
	...
	*********/
    ...
    public function item_costing($item, $pi = NULL) 
	{
        ...
        if ($this->Settings->accounting_method != 2 && !$this->Settings->overselling) {
            ...
            if ($this->site->getProductByID($item['product_id'])) {
                if ($item['product_type'] == 'standard') {
                    $cost = $this->calculateAVCost($item['product_id'], $item['warehouse_id'], $item['net_unit_price'], $item['unit_price'], $item['quantity'], $item['product_name'], $item['option_id'], $item_quantity, (isset($item['transaction_type']) ? $item['transaction_type']: ''), (isset($item['transaction_id'])?$item['transaction_id']:''),(isset($item['status'])?$item['status']:'') );
				} elseif ($item['product_type'] == 'combo') {
                    $combo_items = $this->getProductComboItems($item['product_id'], $item['warehouse_id']);
                    foreach ($combo_items as $combo_item) {
                ...
                foreach ($combo_items as $combo_item) {
                    if ($combo_item->type == 'standard') {
                        if (isset($citems['p' . $combo_item->id . 'o' . $item['option_id']])) {
                            ...
                            $citems['p' . $combo_item->id . 'o' . $item['option_id']]['transaction_type'] = $item['transaction_type'];
                            $citems['p' . $combo_item->id . 'o' . $item['option_id']]['transaction_id']   = $item['transaction_id'];
                            $citems['p' . $combo_item->id . 'o' . $item['option_id']]['aquantity'] = ($combo_item->qty * $item['quantity']);
                        }
                    }
                }
	...
	{
    	$this->db->select("CONCAT(erp_users.first_name,' ',erp_users.last_name) AS saller,CONCAT(erp_users.first_name_kh,' ',erp_users.last_name_kh) AS saller_kh,erp_users.gender,erp_users.date_of_birth,erp_users.nationality,erp_users.nationality_kh,erp_companies.company,erp_companies.company_kh,erp_companies.address,erp_companies.street,erp_companies.village,erp_companies.sangkat,erp_companies.district,erp_companies.state,erp_companies.city,erp_companies.country")
		->from('erp_sales')
	...
	{
    	$this->db->select("(erp_sales.term/erp_sales.frequency) AS duration")
		->from('erp_sales')
		->where('erp_sales.id', $id);
		$q = $this->db->get();
        if ($q->num_rows() > 0) {
	...
	public function updateComboCost($code)
	{
		...
		foreach($combo_id as $combo){
			$comcost = $this->getComboCost($combo->id);
			$this->db->update('products', array('cost' => $comcost->p_cost), array('id' => $combo->id));
		}
	}
