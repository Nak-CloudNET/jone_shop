...
class Auth extends MY_Controller
{
    ...
    {
        $this->erp->checkPermissions('users', TRUE);
    ...
    function create_user()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            // $this->erp->print_arrays($userBankAccounts);exit();
	...
	function update_permission($id = NULL)
	{
		$this->form_validation->set_rules('group', lang("group"), 'is_natural_no_zero');
		if ($this->form_validation->run() === TRUE) {
			$scat = $this->input->post('cate');
			if($scat !=''){
				foreach($scat as $cats){
					$sale_catarr[] = $cats;
				}
				$sales_category = implode(',', $sale_catarr);
			}
			$pcat = $this->input->post('pcate');
			if($pcat !=''){
				foreach($pcat as $catp){
					$purchase_catarr[] = $catp;
				}
				$purchase_category = implode(',', $purchase_catarr);
			}
			$data = array(
				'sales_standard'    => $this->input->post('sales_standard'),
				'sales_combo'       => $this->input->post('sales_combo'),
				'sales_digital'     => $this->input->post('sales_digital'),
				'sales_service'     => $this->input->post('sales_service'),
				'sales_category'    => $sales_category,
				'purchase_standard' => $this->input->post('purchases_standard'),
				'purchase_combo'    => $this->input->post('purchases_combo'),
				'purchase_digital'  => $this->input->post('purchases_digital'),
				'purchase_service'  => $this->input->post('purchases_service'),
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $user->name);
						$this->excel->getActiveSheet()->SetCellValue('F' . $row, $user->astatus);
                        $row++;
                    }
	...
	function checkPassDiscount(){
		$password = $_REQUEST['password'];
		$id = 1;
		$result = $this->ion_auth->hash_password_db($id, $password);
		if($result == 1){
			echo $result;
		}else{
			return false;
