...
//            $this->data['payment_ref'] = $this->site->getReference('sp');
	...
	function tax_invoice1($id = NULL)
    {
		...
		$this->data['logo'] = true;
        $this->load->view($this->theme . 'sale_order/tax_invoice1', $this->data);
    }
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $products[] = array(
                        ...
                        'unit_price' => $this->erp->formatDecimal($unit_price),
                        'quantity' => $item_quantity,
                        'quantity_received' => $item_qty_received,
                        'warehouse_id' => $warehouse_id,
                        'item_tax' => $pr_item_tax,
                        'tax_rate_id' => $pr_tax,
                        'tax' => $tax,
        ...
        if ($rows) {
			...
			//	$this->erp->print_arrays($curr_by_item);
                ...
                if ($row->tax_rate) {
					...
					$pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options,'group_price'=>$group_price,'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_group->percent,'makeup_cost_percent'=>$percent->percent);
        ...
        if ($rows) {
            foreach ($rows as $row) {
                ...
                if ($row->tax_rate) {
                    ...
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options,'group_price'=>$group_price);
                }
            }
			//$this->erp->print_arrays($pr);
            echo json_encode($pr);
	...
	function delivery_tiger($id = NULL)
    {
		...
		$this->data['totaldq'] = $this->sale_order_model->getTotalDeliveryQuantity($inv->sale_reference_no);
		$this->data['row'] = $this->sale_order_model->getDeliveryTigerByID($inv->sale_id);
		//get time delivery
		$datetime = $inv->date;
		$time_arr= explode(" ", $datetime);
		$this->data['time'] = $time_arr[1];
