...
class Quotes extends MY_Controller
{
    ...
    public function add()
    {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    if (isset($item_discount)) {
                        ...
                        if ($dpos !== false) {
                            ...
                            $pr_discount = (($this->erp->formatDecimal($unit_price * $item_quantity)) * ((Float) ($pds[0]) / 100))/$item_quantity;
        ...
        if ($this->form_validation->run() == true && ($quote_id = $this->quotes_model->addQuote($data, $products, $payment))) {
			...
			redirect("quotes/quote_/".$quote_id);
	...
	public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->quotes_model->updateQuote($id, $data, $products, $payment)) {
            ...
            foreach ($inv_items as $item) {
                ...
                if ($row->tax_rate) {
                    $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
                    $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options, 'group_prices' => $group_prices, 'all_group_price' => $all_group_prices);
                } else {
                    $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options, 'group_prices' => $group_prices, 'all_group_price' => $all_group_prices);
                }
                $c++;
            }
        ...
        if ($rows) {
            foreach ($rows as $row) {
                ...
                if ($row->tax_rate) {
					...
					$pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options,'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices ,'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_group->percent,'makeup_cost_percent'=>(isset($percent->percent)?$percent->percent:0));
                } else {
                    $pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options,'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_group->percent,'makeup_cost_percent'=>(isset($percent->percent)?$percent->percent:0));
                }
	...
	function invoice_quotes($quote_id = NULL)
    {
        ...
        $this->data['inv'] = $inv;		
		$this->data['deposit'] = $this->quotes_model->getQuoteDepositByQuoteID($quote_id);	
        $this->load->view($this->theme . 'quotes/invoice_quotes', $this->data);
    }
