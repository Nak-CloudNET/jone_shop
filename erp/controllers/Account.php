  ...
  //       }
			...
			(
				...
				WHEN gt.tran_type = 'SALES' THEN
					(
						...
						FROM
							erp_sales
						LEFT JOIN erp_payments ON erp_sales.id = erp_payments.sale_id
						WHERE
				...
				WHEN gt.tran_type = 'PURCHASES' THEN
					(
						...
						FROM
							erp_purchases
						LEFT JOIN erp_payments ON erp_purchases.id = erp_payments.purchase_id
						WHERE
	...
	public function updateJournal()
	{
		...
		foreach($gltans as $key => $gltran){
			if($gltran->account_code != $account_code[$key]){
			...
			if (!empty($_POST['val'])) {
				...
				if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
					...
					foreach ($_POST['val'] as $id) {
						...
						$this->excel->getActiveSheet()->SetCellValue('A' . $row, $account->date);
						$this->excel->getActiveSheet()->SetCellValue('B' . $row, $account->payment_ref." ");
						$this->excel->getActiveSheet()->SetCellValue('C' . $row, $account->sale_ref." ");
						$this->excel->getActiveSheet()->SetCellValue('D' . $row, $account->customer);
						$this->excel->getActiveSheet()->SetCellValue('E' . $row, $account->noted);
