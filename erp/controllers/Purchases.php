...
class Purchases extends MY_Controller
{
	...
	public function getPurchases($warehouse_id = null)
    {
        ...
        <ul class="dropdown-menu pull-right" role="menu">
           ...
           (($this->Owner || $this->Admin) ? '<li>'.$using_stock.'</li>' : ($this->GP['products-using_stock'] ? '<li>'.$using_stock.'</li>' : '')).
    ...
    '</div></div>';
        ...
        if ($warehouse_id) {
            $this->datatables
                ...
                ->where('purchases.biller_id', $biller_id);
	...
	*********/
			...
			if($type_exp == 'po'){
				...
				for ($r = 0; $r < $i; $r++) {
					...
					if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
						...
						$cos_variant = $item_net_cost + $item_tax;
	...
	function getViewSupplierBalance($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            if ($start_date) {
                $this->db->where($this->db->dbprefix('purchases').'.date BETWEEN "00:00:00' . $start_date . '" and "' . $end_date . '23:59:00"');
            }
	...
	*********/
	...
	public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $due_date                = $payment_term_details ? date('Y-m-d', strtotime('+' . $payment_term_details->due_day . ' days')) : NULL;
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $balance = $sum_grandTotal = $sum_paid = $sum_balance = 0;
                    foreach ($_POST['val'] as $id) {
                        $purchase = $this->purchases_model->getPurchaseByID($id,$wh);
                       //calculate balance
                        $balance = $purchase->grand_total-$purchase->paid;
                        //total sum for each
                        $sum_grandTotal += $purchase->grand_total;
    ...
    public function add_purchase_order($quote_id = null, $sale_order_id= null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_quantity 	= $_POST['quantity'][$r];
				$serial_no 		= isset($_POST['serial'][$r]);
                $p_supplier 	= $_POST['rsupplier_id'][$r];
				$create_id 		= $_POST['create_id'][$r];
				$p_price 		= isset($_POST['price'][$r]);
                $p_type 		= $_POST['type'][$r];
				$tax_method		= $_POST['tax_method'][$r];
                $item_option 	= isset($_POST['product_option'][$r]) ? $_POST['product_option'][$r] : NULL;
				if($item_option == 'undefined'){
