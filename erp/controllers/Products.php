	...
	*/	
    ...
    function print_barcodes($product_id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            // $this->erp->print_arrays($this->products_model->getRate());
    ...
    function add($id = NULL,$param=null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            //$this->erp->print_arrays($data, $items, $product_attributes);
        }

        if ($this->form_validation->run() == true && $pid = $this->products_model->addProduct($data, $items, $product_attributes, $photos, $related_products)) {
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run('products/add') == true) {
            ...
            $data['quantity'] = isset($wh_total_quantity) ? $wh_total_quantity : 0;
            //$this->erp->print_arrays($data, $update_variants, $product_attributes, $photos, $items);
        }

        if ($this->form_validation->run() == true && $this->products_model->updateProduct($id, $data, $items, $product_attributes, $photos, $update_variants, $related_products)) {
	...
	public function add_using_stock()
	{
		...
		if ($this->form_validation->run() == true){
			...
			if(validation_errors()){
				$this->site->updateReference($ref_prefix, $this->input->post('shop'));
			}
	...
	public function get_using_stock()
    {
		...
		$action_link = '<div class="btn-group text-left"><button type="button" class="btn btn-default btn-xs btn-primary dropdown-toggle" data-toggle="dropdown">'.lang("actions").'<span class="caret"></span></button>
					        <ul class="dropdown-menu pull-right" role="menu">					        									
								<li><a href="'.site_url('products/edit_using_stock_by_id/$1/$2').'" ><i class="fa fa-newspaper-o"></i>'.lang('edit_using_stock').'</a></li>															
