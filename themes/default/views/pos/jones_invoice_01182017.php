    ...
    <div id="receipt-data">
        ...
        <div class="text-center">
			...
			<table style="width: 100%; margin-top: 5px; border-bottom: 1px solid black;">
				<tr style="background-color: #CCC !important; border-bottom:1px solid #FFF !important;">
					<td class="text-left"><strong>Total (<?= $default_currency->code; ?>)</strong></td>
					<td style="text-align:right;"><strong>$ <?=$this->erp->formatMoney($inv->total);?></strong></td>
				</tr>
				<tr>
					<td class="text-left">Total Riel (RIEL)</td>
					<td style="text-align:right;">R <?=$this->erp->formatMoney($inv->total*$inv->other_cur_paid_rate)?></td>
				</tr>
				<tr>
				...
				<tr>
					<td class="text-left"><?= lang('vat') ?></td>
				...
				<tr>
					...
					<td style="text-align:right;">$ <?=$this->erp->formatMoney($inv->total - $inv->order_discount)?></td>
				</tr>
				<?php } ?>
				<!-- <?php
			...
			<table class="received" style="width:100%;margin-top: 5px;">
				<tr>
					<td class="text-left" style="background-color: #CCC !important;">
						<?php
							if($inv->paid_by == 'cash'){
								echo "<strong>Cash(USD)</strong>";
					...
					<td style="text-align:right; background-color: #CCC !important;"><strong>$ <?= $this->erp->formatMoney($inv->recieve_usd); ?></strong></td>
				</tr>
				<tr>
					<td class="text-left">
						<?php
					...
					<td style="text-align:right;">R <?= $this->erp->formatMoney($inv->recieve_usd * $inv->other_cur_paid_rate); ?></td>
				</tr>
				<?php
				
				if($inv->paid-($inv->total+$inv->order_tax-$inv->order_discount) >= 0){?>
				<tr>
					<td class="text-left" style="background-color: #CCC !important;"><strong>Change Given (<?= $default_currency->code; ?>)</strong></td>
					<td style="text-align:right; background-color: #CCC !important;"><strong>$ <?=$this->erp->formatMoney($inv->pos_balance)?></strong></td>
				...
				<?php }?>
				<?php if($inv->paid-($inv->total-$inv->order_discount+$inv->order_tax) < 0){?>
					<tr>
					...
					<tr>
						...
						<td style="text-align:right;">R <?=$this->erp->formatMoney($inv->pos_balance * $inv->other_cur_paid_rate)?></td>
					</tr>
				<?php }?>
			</table>
	        ...
	        <div class="col-xs-12 no-print" style="background:#F5F5F5; padding:10px;">
	            <p style="font-weight:bold; text-transform: uppercase; text-align: left;">Please don't forget to disble the header and footer in browser print
	                settings.</p>

	            <p style="text-transform: capitalize; text-align: left"><strong>FF:</strong> File &gt; Print Setup &gt; Margin &amp;
	                Header/Footer Make all --blank--</p>

	            <p style="text-transform: capitalize; text-align: left"><strong>chrome:</strong> Menu &gt; Print &gt; Disable Header/Footer
