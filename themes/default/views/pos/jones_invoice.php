    ...
    <div id="receipt-data">
        ...
        <div class="text-center">
            ...
            echo "<p>" . $biller->address . " " . $biller->city . " " . $biller->postal_code . " " . $biller->state . " " . $biller->country .
                "<br>".$biller->invoice_footer."<br>" . lang("tel") . " : " . $biller->phone.",&nbsp;&nbsp;".lang('email'). " : " . $biller->email; 
				...
				<div class="col-xs-12">	
					<div class="pull-left">
						<table class="text-left">
							...
							//$this->erp->print_arrays($cashier);
							?>
								
							<?php } ?>
							<tr>
								<td><?=lang('Cashier Name');?></td>
								<td>&nbsp;&nbsp;:&nbsp;&nbsp;</td>
            ...
            <table class="table-condensed receipt no_border_btm" style="width:100%;">
				<thead>
					<tr style="border:1px dotted black !important;">
						...
						<th style="text-align:right;width: 100px;"><?= lang("unit_price"); ?></th>
			...
			<table style="width: 100%; margin-top: 5px; border-bottom: 1px solid black;">
				...
				<?php if($inv->order_tax != 0){?>
					<tr>
						...
						<td style="text-align:right;"><?= '('.$tax->name.') $ '. $this->erp->formatMoney($inv->order_tax);?></td>
					</tr>
				<?php }else{?>
					<tr>
                        <td class="text-left"><?=$inv->tax_name?></td>
			...
			<table class="received" style="width:100%;margin-top: 5px;">
				<tr>
					...
					<td style="text-align:right;font-size:16px;"><b>$ <?= $this->erp->formatMoney($inv->grand_total); ?></b></td>
				</tr>
                <?php 
					$pos_paid 	= 0;
					$pos_paidd 	= 0;
				...
				?>
				<table style="width: 100%;">
					<?php if ($pos_paidd > $inv->grand_total) {?>
						<tr>
							...
							<?php
								echo '$ '. $this->erp->formatMoney(abs($pos_paidd - $inv->grand_total));
							?>
							</th>
						</tr>
						<tr>
							<th style="border-top:2px dotted #000;padding-right: 12px;" colspan="<?= $colspan ?>" class="text-right"><?= lang("change_amount_kh"); ?></th>
							<th style="border-top:2px dotted #000" class="text-right"><?= number_format(abs($pos_paidd - $inv->grand_total)*(($pos_settings->in_out_rate) ? $outexchange_rate->rate:$exchange_rate->rate)) . ' ៛' ; ?> </th>
						</tr>
					<?php } ?>
					<?php if ($pos_paidd < $inv->grand_total) {?>
						<tr>
