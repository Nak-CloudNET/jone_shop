	...
	$(document).ready(function(){
		...
		*/
		...
		$('#excel').on('click', function (e) {
            ...
            e.preventDefault();
                window.location.href = "<?= site_url('reports/inventory/0/xls/'.$reference1.'/'.$wahouse_id1.'/'.$product_id1.'/'.$from_date1.'/'.$to_date1.'/'.$stockType1.'/'.$cate_id1) ?>";
                return false;
        });
        $('#pdf').on('click', function (e) {
            e.preventDefault();
                window.location.href = "<?= site_url('reports/inventory/pdf/0'.$reference1.'/'.$wahouse_id1.'/'.$product_id1.'/'.trim($from_date1).'/'.trim($to_date1).'/'.$stockType1.'/'.$cate_id1) ?>";
                return false;  
        });
	});
</script>
