                    ...
                    <table id="tbstock" class="table table-condensed table-bordered table-hover table-striped" >
						...
						?>
							<tr>
								...
								if(is_array($warefull)){
									foreach($warefull as $w){
										$qty = $this->reports_model->getQtyByWare($pro->id,$w->id,$product2,$category2,$from_date2,$to_date2);
										if(isset($qty->wqty)){
											echo "<td  class='text-right'>".$this->erp->formatDecimal($qty->wqty)."<br>".$this->erp->convert_unit_2_string($pro->id,$qty->wqty)."</td>";
											$tt+=$qty->wqty;
										}else{
								...
								?>
								<?php 
									echo "<td class='text-right'><b>".$this->erp->formatDecimal($tt)."</b><br>".$this->erp->convert_unit_2_string($pro->id,$tt)."</td>";
								?>
							</tr>
						<?php
							$tt_qty +=$tt;
