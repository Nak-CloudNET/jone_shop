			...
			if($(".schedule .ch_check:checked").length > 0){
				...
				$(".schedule .ch_check:checked").each(function(){
					...
					//if(parent.find(".paid").val());
						...
						alert(parseFloat((parent.children("td:nth-child(3)").html())));
				});
			
				
				if(id){
					
					$(this).attr('href', "<?= site_url('sales/add_payment_loans') ?>/"+total_payment+"/"+id+"/"+paid_amount+"/"+principle);
				}else{
					alert("Please check..");
					return false;
				}
