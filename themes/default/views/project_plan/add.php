<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        if (!localStorage.getItem('pndate')) {
        ...
        $(document).on('change', '#pndate', function (e) {
            localStorage.setItem('pndate', $(this).val());
        });
        if (pndate = localStorage.getItem('pndate')) {
            $('#pndate').val(pndate);
        }
        $(document).on('change', '#pnbiller', function (e) {
            localStorage.setItem('pnbiller', $(this).val());
			billerChange();
        });
        if (pnbiller = localStorage.getItem('pnbiller')) {
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-plus"></i><?= lang('add_project_plan'); ?></h2>
    </div>
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo form_open_multipart("project_plan/add", $attrib)
                ?>
                ...
                <div class="row">
                    <div class="col-lg-12">
							...
							<div style="float:left;width:100%;">
								<div class="form-group">
									<div class="input-group" style="width:100%">  
										<?php echo form_input('reference_no', "",'class="form-control input-tip" id="pnref"'); ?>
										<input type="hidden"  name="temp_reference_no"  id="temp_reference_no" value="" />
						...
						<?php if ($Owner || $Admin || !$this->session->userdata('biller_id')) { ?>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <?= lang("biller", "pnbiller"); ?>
                                    <?php
                                    $bl[""] = "";
                                    foreach ($billers as $biller) {
                                        $bl[$biller->id] = $biller->company != '-' ? $biller->code .'-'.$biller->company : $biller->name;
                                    }
                                    echo form_dropdown('biller', $bl, (isset($_POST['biller']) ? $_POST['biller'] : $Settings->default_biller), 'id="pnbiller" data-placeholder="' . lang("select") . ' ' . lang("biller") . '" required="required" class="form-control input-tip select" style="width:100%;"');
                                    ?>
                                </div>
                            </div>
                        <?php } else if($this->session->userdata('biller_id')){ ?>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    echo form_dropdown('biller', $bl, (isset($_POST['biller']) ? $_POST['biller'] : $this->session->userdata('biller_id')), 'id="pnbiller" data-placeholder="' . lang("select") . ' ' . lang("biller") . '" required="required" class="form-control input-tip select" style="width:100%;pointer-events: none;"');
                                    ?>
                                </div>
                            </div>
                        <?php } ?>
						...
						<div class="col-md-4">
							<div class="form-group">
								<?= lang("plan", "plan") ?>
								<input id="plan" type="text" name="plan" class="form-control" required/>
							</div>
						</div>
                        ...
                        <div class="row" id="bt">
                            <div class="col-sm-12">
                                <div class="col-sm-12">
                                    <div class="form-group">
                                        <?= lang("note", "pnnote"); ?>
                                        <?php echo form_textarea('note', (isset($_POST['note']) ? $_POST['note'] : ""), 'class="form-control" id="pnnote" style="margin-top: 10px; height: 100px;"'); ?>
                                    </div>
                                </div>
                            </div>
						</div>
...
<script type="text/javascript">
	...
	function billerChange(){
		...
		$.ajax({
            url: '<?= base_url() ?>project_plan/getReferenceByProject/pn/'+id,
            dataType: 'json',
            success: function(data){
                $("#pnref").val(data);
