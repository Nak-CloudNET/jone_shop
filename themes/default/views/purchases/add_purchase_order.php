...
var test = '<?=$this->session->userdata('remove_polso');?>';
    ...
    localStorage.setItem('powarehouse', '<?= isset($quote->warehouse_id); ?>');
    localStorage.setItem('ponote', '<?= str_replace(array("\r", "\n"), "", $this->erp->decode_html(isset($quote->note))); ?>');
    localStorage.setItem('podiscount', '<?= isset($quote->order_discount_id); ?>');
    localStorage.setItem('potax2', '<?= isset($quote->order_tax_id); ?>');
    localStorage.setItem('poshipping', '<?= isset($quote->shipping); ?>');
    localStorage.setItem('poitems', JSON.stringify(<?= $quote_items ?>));
	localStorage.setItem('slbiller', '<?= isset($inv->biller_id);?>');
	localStorage.setItem('slpayment_term', '<?= isset($inv->payment_term); ?>');
    <?php } ?>
	...
	localStorage.setItem('slpayment_term', '<?=$sale_order->payment_term;?>');
    <?php } ?>
	
	<?php if (isset($inv)) { ?>
		
        localStorage.setItem('podate', '<?= $this->erp->hrld($inv->date);?>');
        localStorage.setItem('posupplier', '<?=$inv->supplier_id;?>');
		localStorage.setItem('slbiller', '<?=$inv->biller_id;?>');
        localStorage.setItem('reference_no_request', '<?=$inv->reference_no;?>');
        localStorage.setItem('powarehouse', '<?=$inv->warehouse_id;?>');
        localStorage.setItem('postatus', '<?=$inv->status;?>');
		localStorage.setItem('pur_ref', '<?=isset($inv->purchase_ref);?>');
            ...
            <div class="col-lg-12">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div class="col-md-12">
                            <div
                                class="from-group"><?php echo form_submit('add_pruchase', $this->lang->line("submit"), 'id="add_pruchase" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0; display:none;"'); ?>
	...
	function billerChange(){
        ...
        $.ajax({
            ...
            success: function(result){
				localStorage.setItem('default_warehouse','<?= isset($setting->default_warehouse) ?>');
