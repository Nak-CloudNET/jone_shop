$(document).ready(function () {
	...
	if (localStorage.getItem('pnitems')) {
		loadItems();
	}
    ...
    $('#reset').click(function (e) {
        bootbox.confirm(lang.r_u_sure, function (result) {
            if (result) {
                if (localStorage.getItem('pnitems')) {
                    localStorage.removeItem('pnitems');
                }
                ...
                if (localStorage.getItem('pnnote')) {
                    localStorage.removeItem('pnnote');
                }
                $('#modal-loading').show();
                location.reload();
            }
	...
	$('#pnref').change(function (e) {
		localStorage.setItem('pnref', $(this).val());
	});
	
	if (pnref = localStorage.getItem('pnref')) {
		$('#pnref').val(pnref);
	}
	
	$('#pnbiller').change(function (e) {
		localStorage.setItem('pnbiller', $(this).val());
	});
	
	if (pnbiller = localStorage.getItem('pnbiller')) {
		$('#pnbiller').select2("val", pnbiller);
	}
	...
	$('#pnnote').redactor({
		...
		changeCallback: function (e) {
			var v = this.get();
			localStorage.setItem('pnnote', v);
		}
	});
	
	if (pnnote = localStorage.getItem('pnnote')) {
	...
	$(document).on('click', '.podel', function () {
		...
		if(pnitems.hasOwnProperty(item_id)) { } else {
			localStorage.setItem('pnitems', JSON.stringify(pnitems));
			loadItems();
			return;
		}
    });
    ...
    $(document).on("focus", '.rquantity', function () {
		...
		if(parseFloat(qoh) < parseFloat(new_qty) ){
			row.find('.rquantity').val(old_row_qty);
            bootbox.alert(lang.qty_bigger);
            return;
		}
...
function loadItems() {
    if (localStorage.getItem('pnitems')) {
        ...
        an 		= 1;
        $("#pnTable tbody").empty();
        pnitems = JSON.parse(localStorage.getItem('pnitems'));
        ...
        $.each(pnitems, function () {
			...
			/* Stock In Hand */
			tr_html += '<td class="text-right"><span class="text-right scost" id="sstock_' + row_no + '">' + stock_in_hand + '</span><input type="hidden" value="'+ stock_in_hand +'" class="qoh"/></td>';
			
			/* Stock Received */
			tr_html += '<td><input type="hidden" value=""/><input type="text" value="' + item_qty + '" class="form-control rquantity" name="quantity[]" /><input type="hidden" class="opt_qty" /></td>';
