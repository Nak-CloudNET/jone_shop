    ...
    if (localStorage.getItem('poitems')) {
        ...
        $.each(poitems, function () {
            ...
            if (ds.indexOf("%") !== -1) {
                ...
                if (!isNaN(pds[0])) {
					if(item_tax_method == 0){
						item_discount = formatDecimal((parseFloat(((unit_cost) * parseFloat(pds[0])) / 100)) * item_qty) ;
					}else{
						item_discount = formatDecimal((parseFloat(((unit_cost) * parseFloat(pds[0])) / 100))) ;
					}
				} else {
                    item_discount = formatPurDecimal(ds/item_qty);
                }
            } else {
				if(item_tax_method == 0){
			...
			/* Total */
			total +=  (item_tax_method==0?( (parseFloat(net_unit_cost)* parseFloat(item_qty) - (parseFloat(item_discount)*parseFloat(item_qty)) ) ):( (parseFloat(net_unit_cost)* parseFloat(item_qty)+ (parseFloat(pr_tax_val)* parseFloat(item_qty)  ) - (parseFloat(item_discount)*parseFloat(item_qty)) ) ));
        ...
        if (po_edit) {
	        ...
	        tfoot += '<th class="text-right">'+ formatPurDecimal(total)+'<input name="total_s" type="hidden" class="total_s" value="' + total + '"></th>';
