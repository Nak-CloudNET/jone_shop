...
if (localStorage.getItem('sloitems')) {
	//console.log(localStorage.getItem('sloitems'));return;
	loadItems();
}
...
$('#slwarehouse').change(function (e) {
	...
	//	alert(price);item.row.item_load
		...
		if(sloitems[item_id].group_prices){
			...
			sloitems[item_id].group_prices[0].price_group_id = $('#pgroup_price').val() ? $('#pgroup_price').val() : '';
		}
	 ...
	 * Product Group Price change
	 ...
	 $(document).on('change', '#pgroup_price', function () {
		...
		if(item.all_group_price !== false) {
			$.each(item.all_group_price, function () {
				if(opt ==0){
					$('#pprice').val(parseFloat(default_price).toFixed(2));
					...
					if(this.price_group_id == opt && this.price != 0 && this.price != '' && this.price != null) {
						...
						if(this.rate!=0)
						{
							$('#pprice').val(parseFloat((this.price/this.rate)*1).toFixed(2));
							$("#net_price").text(formatMoney((this.price/this.rate)*1));
						}else{
							$('#pprice').val(parseFloat(this.price).toFixed(2));
							$("#net_price").text(formatMoney(this.price));
						}
		...
		if (count == 1) {
			...
			if ($('#slwarehouse').val() && $('#slcustomer').val()) {
				var sup = $("#slcustomer").val();
				 ...
				 localStorage.setItem('slcustomer', sup);
				 localStorage.setItem('slwarehouse', wh);
				$('#slcustomer').select2("readonly", true);
				$('#slwarehouse').select2("readonly", true);
			} else {
				bootbox.alert(lang.select_above);
...
function loadItems() {
	if (localStorage.getItem('sloitems')) {
		...
		$.each(sloitems, function () {
			...
			// alert(JSON.stringify(item.row.price));
			...
			/* if(is_edit == 0){
				...
				if(site.settings.attributes == 0){
					if(item.row.is_quote==0 || item.row.item_load==0)
					{
			...
			//alert(((pr_tax_val) * item_qty));
			// console.log(item_price +'=='+ item_qty +'=='+ pr_tax_val);
			...
			tr_html += '<td class="text-right"><span class="qoh">'+ formatDecimal(item.row.qoh ? item.row.qoh : 0) +'</span></td>';
