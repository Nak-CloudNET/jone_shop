$(document).ready(function (e) {
	...
	$customer.change(function (e) {
		...
		var customer_id = localStorage.getItem('slcustomer');
		$.ajax({
			...
			success: function(data){
				...
				localStorage.setItem('credit_limit', data.credit_limited);
				localStorage.setItem('cust_balance', data.balance);
			}
		});
...
if (credit_limit = localStorage.getItem('credit_limit')) {
	$("#credit_limit").val(credit_limit);
}
if (cust_balance = localStorage.getItem('cust_balance')) {
...
$('#slpayment_status').change(function (e) {
	...
	var paid_by = localStorage.getItem('paid_by_1');
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data === false) {
					...
					var paid = Number(localStorage.getItem('paid'));
					...
					var deposited = localStorage.getItem('deposited');
...
if (slwarehouse = localStorage.getItem('slwarehouse')) {
	 ...
	 * Edit Row Modal Hanlder
	 ...
	 $(document).on('click', '.edit', function () {
					...
					$.each(item.all_group_price, function () {
						if(gp == 1) {
							...
							if(item.row.price_id == 0) {
								$("<option />", {value: this.price_group_id, text: this.group_name + ' (' + formatDecimal(this.price) + ' '+ this.currency_code +')'}).attr("rate",this.rate).attr("datacurrency_code",this.currency_code).appendTo(opt_group_price);
							}else{
								$("<option />", {value: this.price_group_id, text: this.group_name + ' (' + formatDecimal(this.price) + ' '+ this.currency_code +')'}).attr("rate",this.rate).attr("datacurrency_code",this.currency_code).appendTo(opt_group_price);
							}
						}else{
		...
		$('#pprice').val(parseFloat(unit_price).toFixed(2));
	 ...
	 * Edit Row Method
	 ...
	 $(document).on('click', '#editItem', function () {
		...
		//alert(JSON.stringify(slitems[item_id].group_prices));
		
		if(slitems[item_id].group_prices){
			
			slitems[item_id].group_prices[0].price			= price;
			slitems[item_id].group_prices[0].price_group_id = $('#pgroup_price').val() ? $('#pgroup_price').val() : '';
...
function loadItems() {
	if (localStorage.getItem('slitems')) {
		...
		$.each(slitems, function () {
			...
			if (item_promotion == 1 && (current_date >= start_date && current_date <= end_date)){
				...
				if (owner || admin || sale_price) {
					...
					tr_html += '<input class="form-control text-right rprice_t" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + parseFloat(real_unit_price).toFixed(2) + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="item_cost" name="item_cost[]" type="hidden" value="' + item_cost + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + parseFloat(real_unit_price).toFixed(2) + '">';
				}
			}
			...
			} else*/ if(item_type == 'standard' && parseFloat(item_qty) > parseFloat(item_qoh)) {
				$('#row_' + row_no).addClass('danger');
				if(site.settings.overselling != 1) { $('#add_sale, #edit_sale').attr('disabled', true); }
		...
		var gtotal = parseFloat(((total - formatDecimal(order_discount+deposit)) + shipping) + invoice_tax);
