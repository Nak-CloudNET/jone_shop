...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		if(java_applet == 1) {
			...
			var td_bill_vat = '';
			if(bill_vat){
			   td_bill_vat += '<td>VAT: '+ bill_vat +'</td>';
			}else{
				
			}
			var pos_bill_head = '';
				pos_bill_head += '<table style="width:100% !important; margin-bottom:10px !important;"><tr style="text-align:center"><td><img src="'+ img_url +'/assets/uploads/logos/Jones.png" /></td></tr><tr style="text-align:center"><td>Siem Reap Store</td></tr><tr style="text-align:center"><td>'+ bill_addr +'</td></tr><tr style="text-align:center"><td>'+ invoice_footer +'</td></tr><tr style="text-align:center"><td>Tel : '+ bill_phone + ' , Email : ' + bill_email +'</td></tr></table>';
				
				pos_bill_head += '<table style="width:100% !important;font-family: MS Reference Sans Serif; font-size: 13px;"><tr><td>Date: '+currentDate+'</td><td>Time: '+formattedDate3+'</td></tr><tr>'+ td_bill_vat +'<td></td></tr><tr><td>Table: '+ $("#suspend_name").val() +'</td><td></td></tr><tr><td>Cashier: '+u_username+'</td><td></td></tr><tr><td>Customer: '+$('#select2-chosen-1').text()+'</td</tr><tr><td>Ref no. : '+ reference_no +'</td></tr></table>';
		...
		$.each(positems, function (i, e) {
			...
			if (item_type == 'standard' && item.options !== false) {
				...
				if(n == 1){
				var table_b = '<tr style="font-family: MS Reference Sans Serif; font-size: 13px; height: 40px !important">';
					table_b +=  '				<th style="color: #000000; background-color: #FFFFFF !important; border-left:1px dotted #000; border-top: 1px dotted #000; border-bottom:1px dotted #000; text-align:left">Items name</th>';
					table_b +=  '				<th style="color: #000000; background-color: #FFFFFF !important; border-top: 1px dotted #000; border-bottom:1px dotted #000">Qty</th>';
					table_b +=  '				<th style="color: #000000; background-color: #FFFFFF !important; border-top: 1px dotted #000; border-bottom:1px dotted #000">Unit Price</th>';
					if (parseFloat(dis_arr) > 0) {
						table_b +=  '				<th style="color: #000000; background-color: #FFFFFF !important; border-top: 1px dotted #000; border-bottom:1px dotted #000">Discount</th>';
					}
					table_b +=  '				<th style="color: #000000; background-color: #FFFFFF !important; border-right:1px dotted #000; border-top: 1px dotted #000; border-bottom:1px dotted #000">Amount</th>';
					table_b +=  '</tr>';
				}
				...
				if (parseFloat(dis_arr) > 0) {
					bprTr += '<td style="text-align:center; font-family: MS Reference Sans Serif; font-size: 13px;"> ('+item.row.discount+')'+ currencyFormat(item_discount) +'</td>';
				}
				
				bprTr += '<td style="text-align:right;font-family: MS Reference Sans Serif; font-size: 13px;">'+ formatMoney(bill_amount) +'</td>';
				bprTr += '</tr>';
            ...
            if (postax2 = localStorage.getItem('postax2')) {
                $.each(tax_rates, function () {
                    if (this.id == postax2) {
                        ...
                        if (this.type == 1) {
                            total_tax = (((total - order_discount) * this.rate) / 100);
                            tax_name  = this.name;
                        }
                    }
                });
		...
		if(java_applet == 1) {
			...
			bill_totals += '<tr><td colspan="3" style="font-family: MS Reference Sans Serif; font-size: 13px;">Total Riel (RIEL)</td>';
			...
			bill_totals += '<tr><td colspan="3" style="font-family: MS Reference Sans Serif; font-size: 13px;">Exchange Rates</td>';
			...
			bill_totals += '<tr><td colspan="3" style="font-family: MS Reference Sans Serif; font-size: 13px;">Order Discount</td>';
			...
			bill_totals += '<tr><td colspan="5" style="text-align:center; font-family: MS Reference Sans Serif; font-size: 13px;"><p>Free WIFI access code: '+ wifi_code +'</p></td></tr>';
