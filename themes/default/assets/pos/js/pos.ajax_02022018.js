...
$(document).on('blur', '#suspend_room', function () {
	localStorage.setItem('suspendroom', $(this).val());
	$('#suspend_room').val($(this).val());
});

if (suspendroom = localStorage.getItem('suspendroom')) {
	 ...
	 * ---------------------- */
	 $("#ppdiscount").click(function(e) {
	 	e.preventDefault();
	 	var dval = $('#posdiscount').val() ? $('#posdiscount').val() : '';
	 ...
	 $(document).on('click', '#updateOrderDiscount', function() {
	 	var ds = $('#order_discount_input').val() ? $('#order_discount_input').val() : '';
	 	if (is_valid_discount(ds)) {
	 		$('#posdiscount').val(ds);
	 		localStorage.removeItem('posdiscount');
	 		localStorage.setItem('posdiscount', ds);
...
$('#posdiscount').focus(function () {
	...
	$(document).on('click', '.edit', function () {
		...
		//alert(JSON.stringify(item.row.w_piece));
	 ...
	 * Product option change
	 ...
	 /* $(document).on('change', '#poption_', function () {
	 	var row = $('#' + $('#row_id').val()), opt = $(this).val();
	 	var item_id = row.attr('data-item-id');
	 	var item = positems[item_id];
	 	if(item.options !== false) {
...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		if(java_applet == 1) {
			...
			var reference_no = $('#slref').val();

			var formattedDate = "" + d.getDate() + "/" + (d.getMonth()+1) + "/" + d.getFullYear() + " " + d.getHours() + ":" + d.getMinutes() + " " + amPm;
			var formattedDate2 = "" + d.getDate() + "/" + (d.getMonth()+1) + "/" + d.getFullYear();
			var formattedDate3 = "" + d.getHours() + ":" + d.getMinutes() + " " + amPm;
			...
			var pos_head2 = '</h4><p>Room | Table: <span style="font-size:18px;">'+ $("#suspend_name").val() +'</span><br/>Username: '+u_username+'<br/>Customer: '+$('#select2-chosen-1').text()+'<br>In: '+formattedDate+'<br/></p>';

			var pos_bill_head = '';
				//pos_bill_head += '<table style="width:100% !important; margin-bottom:10px !important;"><tr style="text-align:center"><td><img src="'+ img_url +'/assets/uploads/logos/Jones.png" /></td></tr><tr style="text-align:center"><td>'+ bill_addr +'</td></tr><tr style="text-align:center"><td>Tel: '+ bill_phone +'</td></tr></table>';
				
				pos_bill_head += '<table style="width:100% !important;font-family: MS Reference Sans Serif; font-size: 13px;"><tr><td>Date: '+currentDate+'</td><td>Time: '+formattedDate3+'</td></tr><tr><td>Table: '+ $("#suspend_name").val() +'</td><td></td></tr><tr><td>Cashier: '+u_username+'</td><td></td></tr><tr><td>Customer: '+$('#select2-chosen-1').text()+'</td</tr><tr><td>Ref no. : '+ reference_no +'</td></tr></table>';

			$("#order_span").prepend('<h5 class="center">Order</h5> '+pos_head2);
			
			$("#order_span_food").empty();
			$('#order_span_drink').empty();
			$("#order_span_drink").prepend('<h3 class="center">Order Drinks</h3> '+pos_head2);
			$("#order_span_food").prepend('<h3 class="center">Order Foods</h3> '+pos_head2);
			$("#bill_span").prepend(pos_bill_head);
		...
		$.each(positems, function (i, e) {
			...
			if (site.settings.tax1 == 1) {
				if (pr_tax !== false) {
					if (pr_tax.type == 1) {
						if (item_tax_method == '0') {   
							pr_tax_val = parseFloat(((unit_price) * parseFloat(pr_tax.rate)) / (100 + parseFloat(pr_tax.rate))); 		
							pr_tax_rate = parseFloat(pr_tax.rate) + '%';
						} else {
							pr_tax_val = ((unit_price - formatDecimal(subtotal_discount/item_qty)) * parseFloat(pr_tax.rate)) / 100;
							pr_tax_rate = parseFloat(pr_tax.rate) + '%';
						}
			...
			item_price = item_tax_method == 0 ? (unit_price-pr_tax_val) : (unit_price);
			...
			if(layouts != 3) {
				if(pos_settings.show_product_code == 0) {
					...
					tr_html += '<td class="edit" style="cursor:pointer;"><ul class="enlarges"><li><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="product_type[]" type="hidden" class="rtype" value="' + item_type + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><input name="product_note[]" type="hidden" class="rnote" value="' + pn + '"><span class="sname" id="name_' + row_no + '">' + (item_promotion == 1 ? '<i class="fa fa-check-circle"></i> ' : '') + item_name + ' (' + item_code + ')'+(sel_opt != '' ? ' ('+sel_opt+')' : '')+ (pn != '' ? ' [<span id="get_not">'+pn+'</span>]' : '')  +'</span>';
					if(pos_settings.show_item_img != 0) {
						tr_html += '<span class="showimg" style="width:350px;"><table class="table table-bordered"><tr><th>Image</th>'+(item_type == 'combo' ? '<th>Description</th>' : '')+(item_type == 'combo' ? '<th style="width:115px;">Combo Items</th>' : '')+'</tr><tr><td><a href="'+site.base_url+'assets/uploads/'+picture+'" data-toggle="lightbox"><img src="'+site.base_url+'assets/uploads/'+picture+'" alt="' +item_name+ '" style="width:200px;" class="img-thumbnail" /></a></td>'+(item_type == 'combo' ? '<td>'+item_details+'</td>' : '')+(item_type == 'combo' ? '<td><table>'+combo_+'</table></td>' : '')+'</tr></table></span>';
					}
				}
				...
				if(pos_settings.show_product_code == 0) {
					...
					tr_html += '<td class="edit" style="cursor:pointer;"><ul class="enlarges"><li><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="product_type[]" type="hidden" class="rtype" value="' + item_type + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><input name="product_note[]" type="hidden" class="rnote" value="' + pn + '"><span class="sname" id="name_' + row_no + '">' + item_name + ' (' + item_code + ')'+(sel_opt != '' ? ' ('+sel_opt+')' : '')+ (pn != '' ? ' (<span id="get_not">'+pn+'</span>)' : '')  +'</span>';
            ...
            if (item_promotion == 1){
				...
				var bill_amount = formatMoney(((parseFloat(real_unit_price - subtotal_discount) + parseFloat(pr_tax_val)) * parseFloat(item_qty)));
			} else {   
				//tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(subtotal) + '</span><input type="hidden" name="grand_total[]" value="' + formatMoney(subtotal) + '"></td>'; 
				tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty))-subtotal_discount) + '</span><input type="hidden" name="grand_total[]" value="' + formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty))-subtotal_discount) + '"></td>'; 
			 	var bill_amount = formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty))-subtotal_discount);
			}
			...
			if (item_type == 'standard' && item.options !== false) {
				...
				var bprTr = '<tr class="row_' + item_id + '" data-item-id="' + item_id + '">';
				bprTr += '<td style="width:50% !important; font-family: MS Reference Sans Serif; font-size: 13px; padding-bottom:5px">'+ (an-1) + ' ' + item_name+'</td>';
				bprTr += '<td style="text-align:center; font-family: MS Reference Sans Serif; font-size: 13px;">'+ currencyFormat(item_qty) +'</td>';
				bprTr += '<td style="text-align:center; font-family: MS Reference Sans Serif; font-size: 13px;">'+ currencyFormat(item_price) +'</td>';
				
				if (item.row.discount != 0) {
					bprTr += '<td style="text-align:center; font-family: MS Reference Sans Serif; font-size: 13px;"> ('+item.row.discount+')'+ currencyFormat(product_discount) +'</td>';
				}
				bprTr += '<td style="text-align:right;font-family: MS Reference Sans Serif; font-size: 13px;">'+ formatMoney(subtotal) +'</td>';
				bprTr += '</tr>';
				...
				if(item_type_by!="")
				{
					if(item_type_by=='food' && item_print==0)
					{
						...
						if(item_id && help1 == 0) {
							oprTr1 += '<tr style="border-bottom: 1px solid #000; width:100%"><td style="width:50% !important;">&nbsp;&nbsp;Items name</td><td style="width:50% !important; text-align:right !important">Qty</td></tr>';
							help1 = 1;
						}
						
						oprTr1 += '<tr style="font-weight:bold" class="row_' + item_id + ' order_print_food " itemcode="'+item_code+'" data-item-id="' + item_id + '"><td style="width:50% !important"><span style="font-size:11px !important;">-</span> ' + item_name + (item_note? ' (<span style="font-size:12px !important"><i>'+item_note+'</i></span>)':"") +'</td><td style="width:50% !important; text-align:right !important">' + formatDecimal(item_qty) +'</td></tr>';
					    $("#order-table-food").append(oprTr1);
					   
					}else if(item_type_by=='drink' && item_print==0)
					{
						var oprTr2 = '';
						if(item_id && help2 == 0) {
							oprTr2 += '<tr style="border-bottom: 1px solid #000; width:100%"><td style="width:50% !important;">&nbsp;&nbsp;Items name</td><td style="width:50% !important; text-align:right !important">Qty</td></tr>';
							help2 = 1;
						}
						oprTr2 += '<tr style="font-weight:bold" class="row_' + item_id + ' order_print_drink " itemcode="'+item_code+'" data-item-id="' + item_id + '"><td style="width:50% !important"><span style="font-size:11px !important;">-</span> ' + item_name +  (item_note? ' (<span style="font-size:12px !important"><i>'+item_note+'</i></span>)':"") +'</td><td style="width:50% !important; text-align:right !important">' + formatDecimal(item_qty) +'</td></tr>';
					   $("#order-table-drink").append(oprTr2);
					}
				}else{
					
					var oprTr = '<tr class="row_' + item_id + ' order_print " itemcode="'+item_code+'" data-item-id="' + item_id + '"><td>#'+(an-1)+' ' + item_name + (item_note? '['+item_note+']':"") +'</td><td>' + formatDecimal(item_qty) +'</td></tr>';
				}
				if(n == 1){
		...
		$('.bill_discount').val(order_discount);
		...
		var wifi_code = localStorage.getItem('wifi_code');
		// Totals calculations after item addition
		var gtotal = parseFloat(((total - order_discount) + total_tax ) + shipping);
		...
		var gtotal_kh = parseFloat(formatMoney(gtotal) * kh_rate );
		...
		if(java_applet == 1) {
			...
			bill_totals += '<tr><td colspan="3" style="font-family: MS Reference Sans Serif; font-weight:bold; font-size:13px !important; border-top:2px solid #000">Total (USD)</td>';
			bill_totals += '<td style="text-align:right;font-weight:bold; border-top:2px solid #000"></td><td style="font-family: MS Reference Sans Serif; text-align:right;font-weight:bold;font-size:13px; border-top:2px solid #000">$ '+formatMoney(total)+'</td></tr>';

			bill_totals += '<tr><td colspan="3" style="font-family: MS Reference Sans Serif; font-size: 13px;">Total (RIEL)</td>';
			bill_totals += '<td style="text-align:right;"></td><td style="text-align:right;font-family: MS Reference Sans Serif; font-size: 13px;">R '+formatMoney(total * kh_rate)+'</td></tr>';

			bill_totals += '<tr><td colspan="3" style="font-family: MS Reference Sans Serif; font-size: 13px;">Exchange Rates (RIEL)</td>';
			bill_totals += '<td style="text-align:right;"></td><td style="text-align:right;font-family: MS Reference Sans Serif; font-size: 13px;">R '+formatMoney(kh_rate)+'</td></tr>';

			bill_totals += '<tr><td colspan="3" style="font-family: MS Reference Sans Serif; font-size: 13px;">Discount</td>';
			bill_totals += '<td style="text-align:right;"></td><td style="text-align:right;font-family: MS Reference Sans Serif; font-size: 13px;"> ('+posdiscount+') $ '+formatMoney(order_discount)+'</td></tr>';

			bill_totals += '<tr><td colspan="3" style="font-family: MS Reference Sans Serif; font-size: 13px; border-bottom: 1px solid #000 !important">VAT</td>';
			bill_totals += '<td style="text-align:right; border-bottom: 1px solid #000 !important"></td><td style="text-align:right;font-family: MS Reference Sans Serif; font-size: 13px; border-bottom: 1px solid #000 !important">$ '+formatMoney(total_tax)+'</td></tr>';
			...
			bill_totals += '<td style="text-align:right; border-bottom: 1px solid #000 !important"></td><td style="text-align:right;font-family: MS Reference Sans Serif; font-size: 16px; border-bottom: 1px solid #000 !important"><b>$ '+formatMoney(gtotal)+'</b></td></tr><br>';

			//bill_totals += '<tr><td colspan="3" style="font-family: MS Reference Sans Serif; font-size: 13px;">Cash (USD)</td>';
			//bill_totals += '<td style="text-align:right;"></td><td style="text-align:right;font-family: MS Reference Sans Serif; font-size: 13px;">$ '+ formatMoney(gtotal) +'</td></tr>';

			//bill_totals += '<tr><td colspan="3" style="font-family: MS Reference Sans Serif; font-size: 13px;">Cash (RIEL)</td>';
			//bill_totals += '<td style="text-align:right;"></td><td style="text-align:right;font-family: MS Reference Sans Serif; font-size: 13px;">R '+formatMoney(gtotal*kh_rate)+'</td></tr>';

			bill_totals += '<tr><td colspan="5" style="text-align:center; font-family: MS Reference Sans Serif;"><br><h5>Thank you! Please come again.</h5></td></tr>';
			...
			bill_totals += '<tr><td colspan="5" style="text-align:center; font-family: MS Reference Sans Serif; font-size: 13px;"><p>===============================</p></td></tr>';
			bill_totals += '<tr><td colspan="5" style="text-align:center; font-family: MS Reference Sans Serif; font-size: 13px;"><p>Free WIFI access code: '+ wifi_code +'</p></td></tr><br>';
			...
			if(order_discount > 0) {
				bill_totals += '<tr><td colspan="4" style="text-align:right;">'+lang_discount+'</td><td style="text-align:right;font-family: MS Reference Sans Serif; font-size: 13px;">'+ product_discount +'</td></tr>';
			}
